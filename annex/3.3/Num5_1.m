...
%
% eta=@(x)(x-beta_x).*(1+y.^n+z.^n)./y.^n;
% b=@(x)(x-beta_x).*n.*(1+z.^n)./y./(1+y.^n+z.^n);
% c=@(x)-(x-beta_x).*n.*z.^(n-1)./(1+y.^n+z.^n);
% d=@(x)-kappa.*n.*x.^(n-1)./theta_y./(1+x.^n).^2;
% h=@(x)alpha.*n.*(y./K).^(n-1)./theta_z./K./(1+(y./K).^n).^2;
% 
% eta=@(x)(x-beta_x).*(1+(kappa./(1+x.^n)+beta_y).^n+(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)./(kappa./(1+x.^n)+beta_y).^n;
% b=@(x)(x-beta_x).*n.*(1+(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)./(kappa./(1+x.^n)+beta_y)...,
%     ./(1+(kappa./(1+x.^n)+beta_y).^n+(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n);
% c=@(x)-(x-beta_x).*n.*(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^(n-1)./(1+(kappa./(1+x.^n)+beta_y).^n...,
...
% 参数eta取0-100
% eta.*y.^n./(1+y.^n+z.^n)+beta_x-x;
Naccurate=1e3;%参数eta的精度
fun_max=@(x)1e2.*(kappa./(1+x.^n)+beta_y).^n./(1+(kappa./(1+x.^n)+beta_y).^n+(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n...,
    ./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)+beta_x-x;
