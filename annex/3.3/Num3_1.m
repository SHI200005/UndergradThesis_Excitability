...
%
% z=@(x)alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z;
%
...
%
% b=@(x)-(x-beta_x).*n.*(kappa./(1+x.^n)+beta_y).^(n-1)./(1+(kappa./(1+x.^n)+beta_y).^n+(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n...,
%     ./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n);
% c=@(x)(x-beta_x).*n.*(1+(kappa./(1+x.^n)+beta_y).^n)./(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z)...,
%     ./(1+(kappa./(1+x.^n)+beta_y).^n+(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n);
% d=@(x)-kappa.*n.*x.^(n-1)/theta_y./(1+x.^n).^2;
% h=@(x)alpha.*n.*((kappa./(1+x.^n)+beta_y)./K).^(n-1)./theta_z./K./(1+((kappa./(1+x.^n)+beta_y)./K).^n).^2;

% eta=@(x)(x-beta_x).*(1+(kappa./(1+x.^n)+beta_y).^n+(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)...,
%     ./(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n;
...
fun_max=@(x)1e2.*(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n./(1+(kappa./(1+x.^n)+beta_y).^n...,
    +(alpha.*((kappa./(1+x.^n)+beta_y)./K).^n./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)+beta_x-x;
