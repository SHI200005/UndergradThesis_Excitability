...
% miu=@(x)((1+y.^n+z.^n).*(x-beta_x)-niu.*z.^n)./y.^n; % 最先换miu
% y=@(x)kappa./(1+x.^n)+beta_y;
% z=@(x)alpha./(1+(y./K).^n)+beta_z; % z=啥啥y 先换z 再换y
%
% b=@(x)n.*y.^(n-1).*(miu.*(1+z.^n)-niu.*z.^n)./(1+y.^n+z.^n).^2; % 还没代换呢 吼吼
% c=@(x)n.*z.^(n-1).*(-miu.*y.^n+niu.*(1+y.^n))./(1+y.^n+z.^n).^2;
% d=@(x)-kappa.*n.*x.^(n-1)./theta_y./(1+x.^n).^2;
% h=@(x)-alpha.*n.*(y./K).^(n-1)./theta_z./K./(1+(y./K).^n).^2;
%
% miu=@(x)((1+(kappa./(1+x.^n)+beta_y).^n+(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n).*(x-beta_x)-niu.*(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)...,
%     +beta_z).^n)./(kappa./(1+x.^n)+beta_y).^n;
% 
% b=@(x)n.*(kappa./(1+x.^n)+beta_y).^(n-1).*((((1+(kappa./(1+x.^n)+beta_y).^n+(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n).*(x-beta_x)-...,
%     niu.*(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)./(kappa./(1+x.^n)+beta_y).^n).*(1+(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)...,
%     -niu.*(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)./(1+(kappa./(1+x.^n)+beta_y).^n+(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n).^2;
% c=@(x)n.*(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^(n-1).*(-(((1+(kappa./(1+x.^n)+beta_y).^n+(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n).*...,
%     (x-beta_x)-niu.*(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)./(kappa./(1+x.^n)+beta_y).^n).*(kappa./(1+x.^n)+beta_y).^n...,
...
fun_max=@(x)(1e2.*(kappa./(1+x.^n)+beta_y).^n+niu.*(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)/(1+(kappa./(1+x.^n)+beta_y).^n+(alpha./(1+((kappa./(1+x.^n)+beta_y)./K).^n)...,
    +beta_z).^n)+beta_x-x;
