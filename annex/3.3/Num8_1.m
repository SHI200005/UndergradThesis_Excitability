...
% 
% eta=@(x)(x-beta_x).*(1+(kappa.*x.^n./(1+x.^n)+beta_y).^n+(alpha./(1+((kappa.*x.^n./(1+x.^n)+beta_y)./K).^n)+beta_z).^n);
% b=@(x)-(x-beta_x).*n.*(kappa.*x.^n./(1+x.^n)+beta_y).^(n-1)./(1+(kappa.*x.^n./(1+x.^n)+beta_y).^n+(alpha./(1+((kappa.*x.^n./(1+x.^n)+beta_y)./K).^n)+beta_z).^n);
% c=@(x)-(x-beta_x).*n.*(alpha./(1+((kappa.*x.^n./(1+x.^n)+beta_y)./K).^n)+beta_z).^(n-1)./(1+(kappa.*x.^n./(1+x.^n)+beta_y).^n+...,
%     (alpha./(1+((kappa.*x.^n./(1+x.^n)+beta_y)./K).^n)+beta_z).^n);
% d=@(x)kappa.*n.*x.^(n-1)./theta_y./(1+x.^n).^2;
...
fun_max=@(x)1e2./(1+(kappa.*x.^n./(1+x.^n)+beta_y).^n+(alpha./(1+((kappa.*x.^n./(1+x.^n)+beta_y)./K).^n)+beta_z).^n)+beta_x-x;
