...
% z=@(x)alpha.*(x./K).^n./(1+(x./K).^n)+beta_z;
%
...
%
% b=@(x)(x-beta_x).*n.*(1+(alpha.*(x./K).^n./(1+(x./K).^n)+beta_z).^n)./(kappa.*x.^n./(1+x.^n)+beta_y)...,
%     ./(1+(kappa.*x.^n./(1+x.^n)+beta_y).^n+(alpha.*(x./K).^n./(1+(x./K).^n)+beta_z).^n);
% c=@(x)-(x-beta_x).*n.*(alpha.*(x./K).^n./(1+(x./K).^n)+beta_z).^(n-1)./(1+(kappa.*x.^n./(1+x.^n)+beta_y).^n+(alpha.*(x./K).^n./(1+(x./K).^n)+beta_z).^n);
% d=@(x)kappa.*n.*x.^(n-1)/theta_y./(1+x.^n).^2;
% g=@(x)alpha.*n.*(x./K).^(n-1)./theta_z./K./(1+(x./K).^n).^2;
% eta=@(x)(x-beta_x).*(1+(kappa.*x.^n./(1+x.^n)+beta_y).^n+(alpha.*(x./K).^n./(1+(x./K).^n)+beta_z).^n)./(kappa.*x.^n./(1+x.^n)+beta_y).^n;
...
fun = @(x)1e2.*(kappa.*x.^n./(1+x.^n)+beta_y).^n./(1+(kappa.*x.^n./(1+x.^n)+beta_y).^n+(alpha.*(x./K).^n./(1+(x./K).^n)+beta_z).^n)+beta_x-x; % function
x_max = fzero(fun,0.1);%初始值一定要取小
